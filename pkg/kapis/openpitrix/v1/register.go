...
func AddToContainer(c *restful.Container, ksInfomrers informers.InformerFactory, k8sClient k8s.Client, ksClient versioned.Interface, conf *config.Config) error {
	...
	if conf != nil {
		multiClusterEnabled = conf.MultiClusterOptions.Enable
	}
	handler := newOpenpitrixHandler(ksInfomrers, k8sClient, ksClient, multiClusterEnabled)
