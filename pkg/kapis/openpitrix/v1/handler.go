...
func newOpenpitrixHandler(ksInformers informers.InformerFactory, k8sClient k8s.Client, ksClient versioned.Interface, multiClusterEnabled bool) *openpitrixHandler {
	return &openpitrixHandler{
		openpitrix.NewOpenpitrixOperator(ksInformers, k8sClient, ksClient, multiClusterEnabled),
	}
}
	...
	*createRepoRequest.Workspace = req.PathParameter("workspace")
	...
	repo := v1alpha1.HelmRepo{
		ObjectMeta: metav1.ObjectMeta{
			Name: idutils.GetUuid36(constants.HelmRepoIdPrefix),
		...
		Spec: v1alpha1.HelmRepoSpec{
			...
			Workspace:   *createRepoRequest.Workspace,
			Url:         createRepoRequest.URL,
			SyncPeriod:  0,
			Description: strings.ShortenString(createRepoRequest.Description, 512),

			//TODO add credential
			Credential: v1alpha1.HelmRepoCredential{
				//Password: createRepoRequest.Credential,
			},
		},
	}
...
func (h *openpitrixHandler) DoAppAction(req *restful.Request, resp *restful.Response) {
	...
	appId := strings2.TrimSuffix(req.PathParameter("app"), constants.HelmApplicationAppStoreSuffix)
...
func (h *openpitrixHandler) ListAppVersions(req *restful.Request, resp *restful.Response) {
	...
	reverse := params.GetBoolValueWithDefault(req, params.ReverseParam, false)
	appId := req.PathParameter("app")
	appId = strings2.TrimSuffix(appId, constants.HelmApplicationAppStoreSuffix)
...
func (h *openpitrixHandler) GetAppVersionFiles(req *restful.Request, resp *restful.Response) {
	...
	getAppVersionFilesRequest := &openpitrix.GetAppVersionFilesRequest{}
	if f := req.QueryParameter("files"); f != "" {
		getAppVersionFilesRequest.Files = strings2.Split(f, ",")
	}
...
func (h *openpitrixHandler) ListAppVersionAudits(req *restful.Request, resp *restful.Response) {
	...
	conditions.Match[openpitrix.AppId] = strings2.TrimSuffix(appId, constants.HelmApplicationAppStoreSuffix)
	if versionId != "" {
		conditions.Match[openpitrix.VersionId] = versionId
	}
...
func (h *openpitrixHandler) DescribeApplication(req *restful.Request, resp *restful.Response) {
	...
	app, err := h.openpitrix.DescribeApplication(namespace, applicationId)
...
func (h *openpitrixHandler) DeleteApplication(req *restful.Request, resp *restful.Response) {
	...
	err := h.openpitrix.DeleteApplication(namespace, applicationId)
...
func (h *openpitrixHandler) ListApplications(req *restful.Request, resp *restful.Response) {
	...
	result, err := h.openpitrix.ListApplications(workspace, namespace, conditions, limit, offset, orderBy, reverse)
...
func (h *openpitrixHandler) UpgradeApplication(req *restful.Request, resp *restful.Response) {
	...
	//upgradeClusterRequest.ClusterId
...
func (h *openpitrixHandler) ModifyApplication(req *restful.Request, resp *restful.Response) {
	...
	modifyClusterAttributesRequest.ClusterName = clusterName
...
func (h *openpitrixHandler) ListCategories(req *restful.Request, resp *restful.Response) {
	...
	//					openpitrix.RepoId:     constants.AppStoreRepoId,
	//				},
	//			}, "", false, 0, 0)
	//			if err != nil {
	//				klog.Errorln(err)
