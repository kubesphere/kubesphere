...
func AddToContainer(c *restful.Container, k8sClient kubernetes.Interface, monitoringClient monitoring.Interface) error {
	...
	ws.Route(ws.GET("/namespaces/{namespace}/targets/labels").
		To(h.handleMetricLabelsQuery).
		Doc("List available labels and their values of a metric within a specific time span.").
		Param(ws.PathParameter("namespace", "The name of the namespace.").DataType("string").Required(true)).
		...
		Metadata(restfulspec.KeyOpenAPITags, []string{constants.CustomMetricsTag}).
		Writes(monitoring.MetricLabels{}).
		Returns(http.StatusOK, RespOK, monitoring.MetricLabels{})).
		Produces(restful.MIME_JSON)
