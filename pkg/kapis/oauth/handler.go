...
func newHandler(im im.IdentityManagementInterface, tokenOperator im.TokenManagementInterface, authenticator im.PasswordAuthenticator, loginRecorder im.LoginRecordInterface, options *authoptions.AuthenticationOptions) *handler {
...
func (h *handler) OAuthCallBack(req *restful.Request, resp *restful.Response) {
	...
	if err = h.loginRecorder.RecordLoginLogs(authenticated.Name, iamv1alpha2.LoginRecordSpec{
		SourceIP: iputil.RemoteIp(req.Request),
...
func (h *handler) passwordGrant(username string, password string, req *restful.Request, response *restful.Response) {
	...
	if err != nil {
		if err == im.AuthFailedIncorrectPassword {
			if err := h.loginRecorder.RecordLoginLogs(username, iamv1alpha2.LoginRecordSpec{
				SourceIP: iputil.RemoteIp(req.Request),
	...
	if err = h.loginRecorder.RecordLoginLogs(authenticated.GetName(), iamv1alpha2.LoginRecordSpec{
		SourceIP: iputil.RemoteIp(req.Request),
		Type:     iamv1alpha2.LoginSuccess,
		Reason:   iamv1alpha2.AuthenticatedSuccessfully,
	}); err != nil {
