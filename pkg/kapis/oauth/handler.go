...
func (h *handler) discovery(_ *restful.Request, response *restful.Response) {
	result := ProviderMetadata{
		Issuer:            h.options.Issuer.Host,
		Auth:              h.options.Issuer.Host + root + "/authorize",
		Token:             h.options.Issuer.Host + root + "/token",
		Keys:              h.options.Issuer.Host + root + "/keys",
		UserInfo:          h.options.Issuer.Host + root + "/userinfo",
...
func (h *handler) authorize(req *restful.Request, response *restful.Response) {
	...
	if authenticated == nil || authenticated.GetName() == user.Anonymous {
		...
		http.Redirect(response.ResponseWriter, req.Request, h.options.Issuer.Host, http.StatusFound)
		return
	}
