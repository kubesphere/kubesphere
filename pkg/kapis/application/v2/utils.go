...
func parseHelmRequest(createRequest application.AppRequest) (req application.AppRequest, err error) {
	...
	fillEmptyFields(&createRequest, chartPack)

	if createRequest.AppName != "" && !strings.HasPrefix(createRequest.AppName, chartPack.Metadata.Name) {
		return createRequest, errors.New("appName is not equal to chart name")
	}
...
func fillEmptyFields(createRequest *application.AppRequest, chartPack *chart.Chart) {
	if createRequest.AppName == "" {
		createRequest.AppName = application.GetUuid36(chartPack.Metadata.Name + "-")
	}
