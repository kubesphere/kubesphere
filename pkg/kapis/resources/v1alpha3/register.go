...
func AddToContainer(c *restful.Container, informerFactory informers.InformerFactory, cache cache.Cache) error {
	...
	webservice.Route(webservice.GET("/namespaces/{namespace}/{resources}").
		...
		Param(webservice.QueryParameter(query.ParameterAscending, "sort parameters, e.g. reverse=true").Required(false).DefaultValue("ascending=false")).
		Param(webservice.QueryParameter(query.ParameterOrderBy, "sort parameters, e.g. orderBy=createTime")).
		Param(webservice.QueryParameter(query.ParameterFieldSelector, "field selector used for filtering, you can use the = and != operators with field selectors, e.g. fieldSelector=type=kubernetes.io/dockerconfigjson, multiple separated by comma").Required(false)).
		Returns(http.StatusOK, ok, api.ListResult{}))
