...
type DevOpsProjectLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.DevOpsProject, err error)
	// Get retrieves the DevOpsProject from the index for a given name.
	Get(name string) (*v1alpha1.DevOpsProject, error)
...
// List lists all DevOpsProjects in the indexer.
func (s *devOpsProjectLister) List(selector labels.Selector) (ret []*v1alpha1.DevOpsProject, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.DevOpsProject))
	})
	return ret, err
}

// Get retrieves the DevOpsProject from the index for a given name.
func (s *devOpsProjectLister) Get(name string) (*v1alpha1.DevOpsProject, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("devopsproject"), name)
	}
	return obj.(*v1alpha1.DevOpsProject), nil
}
