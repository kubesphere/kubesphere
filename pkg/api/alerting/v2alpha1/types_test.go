...
func TestParseAlertingRuleQueryParams(t *testing.T) {
	queryParam := "/alerting.kubesphere.io/v2alpha1/rules?name=test-cpu&state=firing&health=ok&sort_field=lastEvaluation&sort_type=desc&label_filters=name~test"
	...
	_, err = ParseAlertingRuleQueryParams(request)
	assert.NoError(t, err)
}

func TestParseAlertQueryParams(t *testing.T) {
	queryParam := "/alerting.kubesphere.io/v2alpha1//alerts?state=firing&abel_filters=name~test"
	req, err := http.NewRequest("GET", fmt.Sprintf("http://localhost?%s", queryParam), nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	request := restful.NewRequest(req)
	_, err = ParseAlertQueryParams(request)
	assert.NoError(t, err)
}
