...
func ClusterRoleUsers(clusterRoleName string) ([]*User, error) {
...
func RoleUsers(namespace string, roleName string) ([]*User, error) {
...
func NamespaceUsers(namespaceName string) ([]*User, error) {
	...
	for _, roleBinding := range roleBindings {
		...
		for _, subject := range roleBinding.Subjects {
			if subject.Kind == v1.UserKind &&
				...
				//clusterRoles, err := GetClusterRoles(user.Username)
				//
...
func GetWorkspaceSimpleRules(clusterRoles []*v1.ClusterRole, workspace string) map[string][]SimpleRule {

	workspaceRules := make(map[string][]SimpleRule, 0)

	clusterSimpleRules := make([]SimpleRule, 0)
	...
	for i := 0; i < len(policy.WorkspaceRoleRuleMapping); i++ {
		rule := SimpleRule{Name: policy.WorkspaceRoleRuleMapping[i].Name}
...
func merge(clusterRules []SimpleRule, rules []SimpleRule) []SimpleRule {
...
func GetClusterRoleSimpleRules(clusterRoles []*v1.ClusterRole) ([]SimpleRule, error) {
...
func GetRoleSimpleRules(roles []*v1.Role, namespace string) (map[string][]SimpleRule, error) {

	rulesMapping := make(map[string][]SimpleRule, 0)
...
func GetRole(namespace string, roleName string) (*v1.Role, error) {
	return roleLister.Roles(namespace).Get(roleName)
}
