...
func splitPath(path string) (searchBase string, cn string) {
	...
	if length > 2 {
		...
		searchBase = fmt.Sprintf("%s,%s", strings.Join(basePath, ","), constants.GroupSearchBase)
	} else if length == 2 {
		searchBase = fmt.Sprintf("cn=%s,%s", paths[0], constants.GroupSearchBase)
		cn = paths[1]
	} else {
		searchBase = constants.GroupSearchBase
		if paths[0] == "" {
			cn = "*"
		} else {
			cn = paths[0]
