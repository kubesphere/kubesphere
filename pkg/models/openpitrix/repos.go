...
var (
	ItemExists = errors.New("item exists")
)

type RepoInterface interface {
	CreateRepo(repo *v1alpha1.HelmRepo) (*CreateRepoResponse, error)
	DeleteRepo(id string) error
	ValidateRepo(id string, request *v1alpha1.HelmRepoCredential) (*ValidateRepoResponse, error)
	...
	//ListRepos(workspace, orderBy string, reverse bool, limit, offset int) (*models.PageableResponse, error)
...
func newRepoOperator(informers externalversions.SharedInformerFactory, ksClient versioned.Interface) RepoInterface {
	return &repoOperator{
		informers:  informers,
...
func (c *repoOperator) DoRepoAction(repoId string, request *RepoActionRequest) error {
	...
	copyRepo.Spec.ReSyncNow = true
	data, err := patch.Data(copyRepo)
	if err != nil {
		klog.Errorf("create patch [%s] failed, error: %s", repoId, err)
		return err
	}
	//repo, err = c.repoClient.HelmRepoes().Patch(context.TODO(), repoId, types.MergePatchType, data, metav1.PatchOptions{})
...
func (c *repoOperator) CreateRepo(repo *v1alpha1.HelmRepo) (*CreateRepoResponse, error) {
	...
	if repo.GetWorkspace() != "" {
		// We need check helm repos in current workspace and global helm repos.
		r, _ := labels.NewRequirement(constants.WorkspaceLabelKey, selection.In, []string{"", repo.GetWorkspace()})
		ls = ls.Add([]labels.Requirement{*r}...)
	}

	items, err := c.repoLister.List(ls)
	...
	for _, exists := range items {
		if exists.GetTrueName() == name {
			klog.Error(ItemExists, "name: ", name)
			return nil, ItemExists
		}
	}

	repo.Spec.Description = strings.ShortenString(repo.Spec.Description, DescriptionLen)
...
func (c *repoOperator) ModifyRepo(id string, request *ModifyRepoRequest) error {
	//repo, err := c.repoClient.HelmRepoes().Get(context.TODO(), id, metav1.GetOptions{})
	...
	if ws != repo.GetWorkspace() {
		return nil
	}

	modified := repo.DeepCopy()

	if request.Description != nil {
		modified.Spec.Description = strings.ShortenString(*request.Description, DescriptionLen)
	}
	if request.URL != nil {
		modified.Spec.Url = *request.URL
	}
	//TODO modify credential

	//TODO we need lock if we want to change name or workspace,
	if request.Name != nil {
		modified.Labels[constants.NameLabelKey] = *request.Name
	}
	if request.Workspace != nil {
		repo.Labels[constants.WorkspaceLabelKey] = *request.Workspace
	}

	patch := client.MergeFrom(repo)
	modified.Spec.ReSyncNow = true
