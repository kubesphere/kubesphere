...
import (
	...
	"kubesphere.io/kubesphere/pkg/models/servicemesh/metrics/graph"
)
...
//   (presumably removed from K8S). (metrics-621)
//   - this includes "unknown"
// - service nodes that are not service entries (metrics-1526) and for which there is no incoming
//   error traffic and no outgoing edges (metrics-1326).
// Name: deadNode
...
func (a DeadNodeAppender) applyDeadNodes(trafficMap graph.TrafficMap, globalInfo *GlobalInfo, namespaceInfo *NamespaceInfo) {
	...
	for id, n := range trafficMap {
		...
		DefaultCase:
			...
			// - note: unknown is not saved by this rule (metrics-2078) - i.e. unknown nodes can be declared dead
