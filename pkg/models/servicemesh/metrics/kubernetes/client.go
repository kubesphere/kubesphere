...
import (
	...
	kialiConfig "kubesphere.io/kubesphere/pkg/models/servicemesh/metrics/config"
...
type IstioClient struct {
	...
	// isOpenShift private variable will check if metrics is deployed under an OpenShift cluster or not
	// It is represented as a pointer to include the initialization phase.
	// See kubernetes_service.go#IsOpenShift() for more details.
	isOpenShift *bool
	// Cache controller is a global cache for all k8s objects fetched by metrics in multiple namespaces.
