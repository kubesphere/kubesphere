...
func GetComponents() (map[string]interface{}, error) {
	...
	namespaces := []string{constants.KUBESYSTEM, constants.OPENPITRIX, constants.ISTIO, constants.KUBESPHERE}
	for _, ns := range namespaces {

		if ns != constants.KUBESYSTEM {
		...
		if len(servicelists.Items) > 0 {
			...
			for _, service := range servicelists.Items {
				...
				case constants.KUBESYSTEM:
					count.KubernetesCount++
				case constants.OPENPITRIX:
					count.OpenpitrixCount++
				case constants.KUBESPHERE:
