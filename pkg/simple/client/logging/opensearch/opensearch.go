...
import (
	...
	"kubesphere.io/kubesphere/pkg/simple/client/opensearch"
...
// Opensearch implement logging interface
type client struct {
	c *opensearch.Client
}
...
func NewClient(options *logging.Options) (logging.Client, error) {
	...
	var err error
	c.c, err = opensearch.NewClient(options.Opensearch.Host, options.Opensearch.BasicAuth, options.Opensearch.Username, options.Opensearch.Password, options.Opensearch.IndexPrefix, options.Opensearch.Version)
