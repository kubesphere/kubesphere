...
import (
	...
	k8sclient "kubesphere.io/kubesphere/pkg/simple/client/k8s"
...
func DescribeResourcesNode(request *restful.Request, response *restful.Response) {
	...
	nodeList, err := k8sclient.Client().CoreV1().Nodes().List(metaV1.ListOptions{LabelSelector: labelSelector})
	if err != nil {
...
func DescribeResourcesWorkload(request *restful.Request, response *restful.Response) {
	...
	case "deployment":
		deploymentList, err := k8sclient.Client().ExtensionsV1beta1().Deployments(namespace).List(metaV1.ListOptions{LabelSelector: labelSelector})
		if err != nil {
	...
	case "statefulset":
		statefulsetList, err := k8sclient.Client().AppsV1().StatefulSets(namespace).List(metaV1.ListOptions{LabelSelector: labelSelector})
		if err != nil {
	...
	case "daemonset":
		daemonsetList, err := k8sclient.Client().ExtensionsV1beta1().DaemonSets(namespace).List(metaV1.ListOptions{LabelSelector: labelSelector})
		if err != nil {
