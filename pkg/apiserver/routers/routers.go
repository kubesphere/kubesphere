...
func V1Alpha2(ws *restful.WebService) {
	...
	ws.Route(ws.GET("/users/{username}/routers").To(getAllRoutersOfUser).
		...
		Metadata(restfulspec.KeyOpenAPITags, tags).
		Param(ws.PathParameter("username", "").
			DataType("string")).
		Writes(v1.Service{}))
...
func createRouter(request *restful.Request, response *restful.Response) {
	...
	if err != nil {
		errors.HandleError(errors.New(errors.InvalidArgument, "wrong annotations, missing key or value"), response)
		return
	}
...
func updateRouter(request *restful.Request, response *restful.Response) {
	...
	if err != nil {
		errors.HandleError(errors.New(errors.InvalidArgument, err.Error()), response)
		return
	}
