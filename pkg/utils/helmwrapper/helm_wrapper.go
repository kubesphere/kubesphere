...
type helmI interface {
	...
	//List all revisions of the release
	//List() ([]helmRlsStatus, error)
	Manifest() (string, error)
}

//func (c *helmWrapper) List() (status []helmRlsStatus, err error) {
...
//			"list",
//			"--filter",
//			fmt.Sprintf("^%s$", c.ReleaseName), //a regular expression (Perl compatible)
//			"--namespace",
//			c.Namespace,
//			"--output",
//			"json",
...
//		return
//	} else {
//		klog.Infof("namespace: %s, name: %s, run command success", c.Namespace, c.ReleaseName)
...
func NewHelmWrapper(kubeconfig, ns, rls string) *helmWrapper {
	c := &helmWrapper{
...
func (c *helmWrapper) ensureWorkspace() error {
	...
	if len(c.Kubeconfig) > 0 {
		kubeFile, err := os.Create(c.kubeConfigPath())
		if err != nil {
			return err
		}
...
func (c *helmWrapper) install(chartName, chartData, values string, upgrade bool) (res HelmRes, err error) {
	if klog.V(2) {
		start := time.Now()
		defer func() {
			klog.Infof("run command end, namespace: %s, name: %s elapsed: %v", c.Namespace, c.ReleaseName, time.Now().Sub(start))
		}()
	}
