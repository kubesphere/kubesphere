...
func (r *InstallPlanReconciler) getOverrides(mainChart *chart.Chart, tag string, cluster *clusterv1alpha1.Cluster) (overrides []string) {
	...
	if r.ExtensionOptions != nil {
		...
		if r.ExtensionOptions.Ingress != nil {
			overrides = append(overrides, "global.extension.ingress.ingressClassName="+r.ExtensionOptions.Ingress.IngressClassName)
			overrides = append(overrides, "global.extension.ingress.domainSuffix="+r.ExtensionOptions.Ingress.DomainSuffix)
			overrides = append(overrides, fmt.Sprintf("global.extension.ingress.httpPort=%d", r.ExtensionOptions.Ingress.HTTPPort))
			overrides = append(overrides, fmt.Sprintf("global.extension.ingress.httpsPort=%d", r.ExtensionOptions.Ingress.HTTPSPort))
		}
	}
...
func (r *InstallPlanReconciler) newExecutor(plan *corev1alpha1.InstallPlan) (helm.Executor, error) {
	executorOptions := []helm.ExecutorOption{
		...
		helm.SetExecutorBackoffLimit(1),
	}
