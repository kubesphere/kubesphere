...
func (r *ReconcileHelmRepo) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	retryAfter := 0
	if need, after := needReSyncNow(copyInstance); need {
		//sync repo
		...
		if err != nil {
			...
			if copyInstance.Spec.ReSyncNow == true {
				//update spec
				patch := client.MergeFrom(copyInstance)
				copyI := copyInstance.DeepCopy()
				copyI.Spec.ReSyncNow = false
				err = r.Client.Patch(context.TODO(), copyI, patch)
				if err != nil {
					klog.Infof("patch data failed, error: %v", err)
					return reconcile.Result{}, err
				}
			}
		}
...
// if resync is false and after > 0, ti should resync in after seconds
func needReSyncNow(instance *v1alpha1.HelmRepo) (syncNow bool, after int) {
	...
	if instance.Spec.ReSyncNow == true && failedTimes == 0 {
...
func (r *ReconcileHelmRepo) syncRepo(instance *v1alpha1.HelmRepo) error {
	...
	if len(instance.Status.Data) != 0 {
		err = json.Unmarshal([]byte(instance.Status.Data), &existsSavedIndex)
		if err != nil {
	...
	data, err := json.Marshal(savedIndex)
	...
	instance.Status.Data = strfmt.Base64(data)
	return nil
}
