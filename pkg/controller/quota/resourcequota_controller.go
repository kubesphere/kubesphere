...
func (r *Reconciler) syncQuotaForNamespaces(originalQuota *quotav1alpha2.ResourceQuota) error {
	...
	for _, namespace := range matchingNamespaceList.Items {
		...
		namespaceTotals, namespaceLoaded := getResourceQuotasStatusByNamespace(quota.Status.Namespaces, namespaceName)
		if !sliceutil.HasString(matchingNamespaceNames, namespaceName) && namespaceLoaded {
		...
		actualUsage, err := quotaUsageCalculationFunc(namespaceName, quota.Spec.Scopes, quota.Spec.Hard, r.registry, quota.Spec.ScopeSelector)
		if err != nil {
			return err
		}
	...
	quota.Status.Total.Hard = quota.Spec.Hard
