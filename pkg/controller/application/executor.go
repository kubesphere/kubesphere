...
func (r *AppReleaseReconciler) uninstall(ctx context.Context, rls *appv2.ApplicationRelease) (jobName string, err error) {
	executor, err := r.getExecutor(rls)
	if err != nil {
	...
	c, err := r.clusterClientSet.Get(rls.GetRlsCluster())
	if err != nil {
		klog.Errorf("failed to get cluster %s, err: %v", rls.GetRlsCluster(), err)
		return jobName, err
	}

	options := []helm.HelmOption{
		helm.SetNamespace(rls.GetRlsNamespace()),
		helm.SetKubeconfig(c.Spec.Connection.KubeConfig),
	}
...
func (r *AppReleaseReconciler) createOrUpgradeAppRelease(ctx context.Context, rls *appv2.ApplicationRelease) error {

	clusterName := rls.GetRlsCluster()
	klog.Infof("begin to create or upgrade %s app release %s in cluster %s ns: %s", rls.Spec.AppType, rls.Name, clusterName, rls.GetRlsNamespace())
	...
	if rls.Spec.AppType == appv2.AppTypeHelm {
		...
		if err != nil && err.Error() == "release: not found" {
			klog.Infof("release %s not found, begin to create %s", rls.Name, err)
		}
		if err == nil {
...
func (r *AppReleaseReconciler) getExecutor(apprls *appv2.ApplicationRelease) (executor helm.Executor, err error) {
	kubeConfig, runClient, dynamicClient, err := r.getClusterInfo(apprls.GetRlsCluster())
	if err != nil {
	...
	return r.getYamlInstaller(runClient, dynamicClient, apprls)
}

func (r *AppReleaseReconciler) getYamlInstaller(runClient client.Client, dynamicClient *dynamic.DynamicClient, apprls *appv2.ApplicationRelease) (executor helm.Executor, err error) {
