...
func TestRequestSetsCluster(t *testing.T) {
	r := NewRequest(&RESTClient{base: &url.URL{Path: "/"}, versionedAPIPath: "/api/v1"}).
		Cluster("host").Resource("nodes")
	if r.cluster == "" {
		t.Errorf("cluster should be set: %#v", r)
	}
	if s := r.URL().String(); s != "/clusters/host/api/v1/nodes" {
		t.Errorf("cluster should be in path: %s", s)
	}
}

func TestRequestSetsWorkspace(t *testing.T) {
	r := (&Request{
		c: &RESTClient{base: &url.URL{Path: "/"}},
	}).Workspace("foo")
	if r.workspace == "" {
		t.Errorf("workspace should be set: %#v", r)
	}

	if s := r.URL().String(); s != "workspaces/foo" {
		t.Errorf("workspace should be in path: %s", s)
	}
}
