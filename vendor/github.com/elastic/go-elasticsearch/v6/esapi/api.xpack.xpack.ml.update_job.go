...
func newXPackMLUpdateJobFunc(t Transport) XPackMLUpdateJob {
	return func(job_id string, body io.Reader, o ...func(*XPackMLUpdateJobRequest)) (*Response, error) {
		var r = XPackMLUpdateJobRequest{JobID: job_id, Body: body}
		for _, f := range o {
			f(&r)
		}
		return r.Do(r.ctx, t)
...
//
func (r XPackMLUpdateJobRequest) Do(ctx context.Context, transport Transport) (*Response, error) {
	var (
		method string
	...
	path.Grow(1 + len("_xpack") + 1 + len("ml") + 1 + len("anomaly_detectors") + 1 + len(r.JobID) + 1 + len("_update"))
	path.WriteString("/")
...
func (f XPackMLUpdateJob) WithContext(v context.Context) func(*XPackMLUpdateJobRequest) {
	return func(r *XPackMLUpdateJobRequest) {
		r.ctx = v
	}
}
