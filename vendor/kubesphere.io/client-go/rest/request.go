...
type Request struct {
	...
	// structural elements of the request that are part of the KubeSphere API conventions
	cluster      string
	group        string
	version      string
	workspace    string
...
// Group applies the group scope to a request
func (r *Request) Group(group string) *Request {
	if r.err != nil {
		return r
	}
	if msgs := IsValidPathSegmentName(group); len(msgs) != 0 {
		r.err = fmt.Errorf("invalid group %q: %v", group, msgs)
		return r
	}
	r.group = group
	return r
...
// Version applies the version scope to a request
func (r *Request) Version(version string) *Request {
	if r.err != nil {
		return r
	}
	if msgs := IsValidPathSegmentName(version); len(msgs) != 0 {
		r.err = fmt.Errorf("invalid version %q: %v", version, msgs)
		return r
	}
	r.version = version
	return r
...
// Workspace applies the workspace scope to a request
func (r *Request) Workspace(workspace string) *Request {
	if r.err != nil {
		return r
	}
	if msgs := IsValidPathSegmentName(workspace); len(msgs) != 0 {
		r.err = fmt.Errorf("invalid workspace %q: %v", workspace, msgs)
		return r
	}
...
// Cluster applies the cluster scope to a request
...
func (r *Request) URL() *url.URL {
	...
	if len(r.cluster) > 0 {
		p = path.Join("/clusters", r.cluster, p)
	}
	if len(r.group) > 0 {
		p = path.Join(p, r.group)
	}
	if len(r.version) > 0 {
		p = path.Join(p, r.version)
	}
	if len(r.namespace) == 0 && len(r.workspace) > 0 {
		p = path.Join(p, "workspaces", r.workspace)
	}
