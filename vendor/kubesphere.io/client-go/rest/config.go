...
type Config struct {
	...
	// proxy to easily relocate all the apiserver endpoints.
	Host string
...
// SetKubeSphereDefaults sets default values on the provided client config for accessing the
// Kubernetes API or returns an error if any of the defaults are impossible or invalid.
func SetKubeSphereDefaults(config *Config) error {
	if len(config.UserAgent) == 0 {
		config.UserAgent = DefaultKubeSphereUserAgent()
	}
	return nil
}
...
func buildUserAgent(command, version, os, arch, commit string) string {
	return fmt.Sprintf(
		"%s/%s (%s/%s) kubesphere/%s", command, version, os, arch, commit)
}

// DefaultKubeSphereUserAgent returns a User-Agent string built from static global vars.
func DefaultKubeSphereUserAgent() string {
...
func AddUserAgent(config *Config, userAgent string) *Config {
	fullUserAgent := DefaultKubeSphereUserAgent() + "/" + userAgent
